...
public interface UserBookReadRepository extends JpaRepository<UserBookRead, Long>{
    ...
    @Query("SELECT ubr FROM UserBookRead ubr WHERE ubr.book.id = :bookId AND ubr.user.id IN :userIds")
    List<UserBookRead> findByBookIdAndUserIds(Long bookId, List<Long> userIds); 
}
